...
const teachers = [
  ...
  { id: 8, name: "Ms. Andersson", subject: "Counselor", email: "counselor@iths.se", avatar: "A", color: "#8b5cf6" }
];

const tutors = [
  { id: 1, name: "Alex", subject: "Math", availability: "Mon, Wed, Fri", desc: "Helps with algebra, calculus, and exam prep.", color: "#3b82f6" },
  { id: 2, name: "Sofia", subject: "English", availability: "Tue, Thu", desc: "Supports essays, reading comprehension, and presentation practice.", color: "#ec4899" },
  { id: 3, name: "Noah", subject: "Science", availability: "Mon, Thu", desc: "Focuses on biology, chemistry, physics and lab concepts.", color: "#22c55e" },
  { id: 4, name: "Emma", subject: "History", availability: "Wed, Fri", desc: "Provides study plans and clear notes for history exams.", color: "#f59e0b" },
  { id: 5, name: "Mia", subject: "Spanish", availability: "Tue, Sat", desc: "Offers conversation practice and grammar review.", color: "#a78bfa" }
];
...
function loadData() {
  ...
  if (saved) {
    ...
    userData.checkins = parsed.checkins || [];
    userData.messages = parsed.messages || [];
    userData.resources = parsed.resources || [];
    userData.osis = parsed.osis || "";
    userData.name = parsed.name || "ITHS student";
  }
  ...
  if (level.level === "High") {
    ...
    if (score >= 70) {
      suggestions.push({
        type: "urgent",
        icon: "🛟",
        text: "Your score is very high. Visit the counselor page for immediate support.",
        action: "switchView('help')"
      });
    }
  }
...
function switchView(viewName) {
  ...
  if (viewName === 'tutoring') updateTutoring();
...
function submitCheckin() {
  ...
  if (level.level === 'High') {
    actions.innerHTML = `
      ...
      <button class="btn-secondary" onclick="switchView('help'); closeModal('modal-result')">Go to Counselor Page</button>
    `;
  ...
  if (score >= 70) {
    setTimeout(() => {
      ...
      switchView('help');
      showToast('High stress detected — you have been redirected to counselor support.', 'warn');
    }, 1200);
  }
}
...
function updateTutoring() {
  const grid = $('tutor-grid');
  const latest = userData.checkins[userData.checkins.length - 1];
  const recommendedSubject = latest?.classes?.[0] || 'Math';
  const recommended = tutors.filter(t => t.subject === recommendedSubject);
  const otherTutors = tutors.filter(t => t.subject !== recommendedSubject);
  ...
  const cards = [
    ...
    <div class="tutor-card">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
        <div>
          <h3>${t.name}</h3>
          <div style="font-size:0.8rem;color:var(--text-muted);">${t.subject} tutor</div>
        </div>
        <div style="font-weight:700;color:${t.color};">${t.availability}</div>
      </div>
      <p>${t.desc}</p>
      <button onclick="requestTutor(${t.id})">Request Session</button>
    </div>
  `).join('');

  if (!grid) return;
  grid.innerHTML = cards || '<div class="empty-state">No tutors are available right now.</div>';
}
...
function requestTutor(tutorId) {
  ...
  const message = {
    id: Date.now(),
    teacherId: teachers.find(t => t.subject === tutor.subject)?.id || null,
    text: `Hi ${tutor.name}, I'd like to schedule a tutoring session for ${tutor.subject}.`, 
    isAnon: false,
    date: new Date().toISOString()
  };
  ...
  showToast(`Tutoring session requested with ${tutor.name}!`, 'success');
  switchView('messages');
}

function requestCounselor() {
  const counselor = teachers.find(t => t.subject === 'Counselor');
  if (!counselor) return;
  selectTeacher(counselor.id);
  showToast('Counselor selected. You can send a message now.', 'success');
  switchView('messages');
}
...
function setUserIdentity() {
  const displayValue = userData.osis || userData.name;
  const profile = $('profile-name');
  const greeting = $('greeting-name');
  const dashboardOsis = $('dashboard-osis');
  const viewSpans = document.querySelectorAll('.view-account-span');
  if (profile) profile.textContent = displayValue;
  if (greeting) {
    if (userData.osis && userData.name !== 'ITHS student') {
      greeting.textContent = userData.name + ' (OSIS: ' + userData.osis + ')';
    } else {
      greeting.textContent = displayValue;
    }
  }
  if (dashboardOsis) dashboardOsis.textContent = userData.osis || 'Not logged in';
  viewSpans.forEach(span => span.textContent = displayValue);
}
...
function loginStudent() {
  const osisInput = $('inp-osis');
  const nameInput = $('inp-name');
  const osis = osisInput?.value.trim();
  const name = nameInput?.value.trim();

  if (!osis) {
    showToast('Please enter your OSIS number', 'error');
    return;
  }
  if (!name) {
  ...
  if (!/^\d{9}$/.test(osis)) {
    showToast('OSIS must be exactly 9 digits', 'error');
    return;
  }
...
function generateAIResponse(userMessage) {
  ...
  if (!message) {
    return `I'm here to help when you're ready. Tell me how you're feeling or what you'd like support with.`;
  }
  ...
  if (message.includes('stress') || message.includes('anxious') || message.includes('worried') || message.includes('overwhelmed')) {
    const responses = [
      ...
      `${context}If stress feels overwhelming, it can help to make a short list of the top 2 things you can solve today and leave the rest for later. This kind of small win can ease pressure and help you feel more in control.`,
      `${context}When stress is high, your brain often needs a reset. Try stepping away from work for 10 minutes—stretch, drink water, or take a walk. Then come back with a simpler task first to rebuild momentum.`
    ];
  ...
  if (message.includes('sleep') || message.includes('tired') || message.includes('exhausted') || message.includes('insomnia')) {
    const responses = [
      ...
      `If bedtime feels far away, try a short wind-down ritual: journaling for 5 minutes, sipping warm water, or listening to calm music. ${context}Consistency is the key, even if you start with small changes.`,
      `When your body is tired, your mind is less able to solve problems. ${context}If possible, set a gentle bedtime goal and practice the same wake-up time tomorrow to help your sleep schedule stabilize.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('study') || message.includes('homework') || message.includes('exam') || message.includes('test') || message.includes('learn')) {
    const responses = [
      ...
      `${context}When exams are close, create a simple study plan with the most important topics first. Review the hardest material while your mind is freshest, and then use shorter review sessions later.`,
      `If homework feels overwhelming, it can help to break it into 3 parts: understand the goal, do the main work, and then review what you learned. ${context}This makes large tasks feel more manageable.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('sad') || message.includes('depressed') || message.includes('mood') || message.includes('happy') || message.includes('feeling')) {
    const responses = [
      ...
      `${context}Sometimes the smallest positive action can shift your mood—drink water, step outside, or send a message to someone you trust. What do you think could help you feel a little better right now?`,
      `If you're feeling down, it may help to give yourself permission to rest and then try one simple thing you enjoy. ${context}You don't need to be productive all the time; rest is also part of taking care of yourself.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }

  // Confidence and self-belief responses
  if (message.includes('confidence') || message.includes('nervous') || message.includes('scared') || message.includes('afraid') || message.includes('self doubt') || message.includes('self-doubt')) {
    const responses = [
      `${context}Feeling nervous is normal, especially when things matter. Try focusing on one small step you can take right now instead of the whole task. That can help you feel calmer and more confident.`,
      `Confidence builds through small successes. ${context}Set a tiny goal you know you can complete, then celebrate it. This will help you feel more capable over time.`,
      `If you're doubting yourself, remember that everyone learns at their own pace. ${context}Focus on your progress rather than perfection, and ask for support when you need it.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('motivation') || message.includes('lazy') || message.includes('procrastinat') || message.includes('focus')) {
    const responses = [
      ...
      `${context}When your energy feels low, it may help to set one meaningful goal for the next 30 minutes and celebrate when you complete it. Small progress often leads to more motivation.`,
      `If you're feeling stuck, try changing your environment: move to a different room, clear your desk, or study with soft background music. ${context}A fresh space can refresh your mindset too.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('friend') || message.includes('social') || message.includes('lonely') || message.includes('relationship')) {
    const responses = [
      ...
      `${context}If you're feeling lonely, reaching out to one person can make a big difference. A simple message or asking someone to study together can help you feel more supported.`,
      `Healthy relationships take time and care. ${context}Try to notice the people who make you feel safe and valued, and consider sharing how you're feeling with them.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }

  // Organization and time management responses
  if (message.includes('time') || message.includes('schedule') || message.includes('organize') || message.includes('planner') || message.includes('manage')) {
    const responses = [
      `${context}A simple schedule can reduce stress quickly. Try writing down the three most important tasks for today, then rank them from easiest to hardest. This helps you stay focused and feel more in control.`,
      `Time management isn't about doing everything at once. ${context}Break your day into blocks: study, rest, meals, and movement. Even a rough plan can make your day feel more manageable.`,
      `If planning feels overwhelming, start with a 5-minute review at the beginning of your day. ${context}Ask yourself: what are the two most important things I need to do? Then work toward them step by step.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('burnout') || message.includes('burnt out') || message.includes('self-care') || message.includes('balance')) {
    const responses = [
      ...
      `If you're feeling burned out, try swapping one task for a restorative activity today: rest, movement, or a creative break. ${context}Recovery is part of success, not something separate from it.`,
      `Burnout often grows when expectations outpace energy. ${context}It can help to say no to one thing and yes to one small renewal action.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
  ...
  if (message.includes('teacher') || message.includes('help') || message.includes('support') || message.includes('counselor')) {
    const responses = [
      `${context}Reaching out for help is a sign of strength, not weakness. Your teachers and school counselors are here to support you. Consider messaging a teacher about your concerns, or visiting the school wellness center. What kind of support are you looking for?`,
      `${context}If you're not sure where to start, you can use the Messages tab to send a quick note to a teacher. If stress feels high, the counselor page is a good place to begin.`,
      `Need help finding the right person to talk to? ${context}Try choosing the subject or concern that feels most urgent, and I can help guide you toward the best support option.`
    ];
    return responses[Math.floor(Math.random() * responses.length)];
  }
